(function (global) {
  ...
  global.SBTIQuizData = {
    id: "sbti",
    mode: "sequential-choice",
    title: "SBTI 人格测试",
    kicker: "15维度趣味测评",
    eyebrow: "Self Behavior Type Indicator",
    subtitle: "30道题 · 15个维度 · 测出你独一无二的人格代号",
    description: "基于自我、情感、态度、行动和社交五大模型的娱乐型人格测试。",
    status: "ready",
    badgeText: "经典主站测试",
    storageKey: "quiz-result:sbti:v1",
    shareLabel: "复制站点链接",
    heroStats: [
      { value: "30+", label: "核心题目" },
      { value: "15", label: "人格维度" },
      { value: "娱乐向", label: "解读风格" }
    ],
    introItems: [
      { title: "测什么？", text: "你的自我认知、情绪模式、规则感、行动习惯和社交边界。" },
      { title: "怎么测？", text: "做完所有题目后，系统会把十五维向量映射到最接近的人格代号。" },
      { title: "结果风格", text: "解读偏损但不当真，纯娱乐，不做任何医学或心理诊断。" }
    ],
    getInitialState() {
      return { currentIndex: 0, answers: {}, shuffledQuestions: [] };
    },
    prepareState(state) {
    ...
    getQuestionMeta(question) {
      ...
      return dimensionMeta[question.dim].name;
    },
    getQuestionCount(state) {
      return this.getVisibleQuestions(state).length;
    },
    getCurrentQuestion(state) {
      return this.getVisibleQuestions(state)[state.currentIndex];
    },
    updateAnswer(state, questionId, value) {
      state.answers[questionId] = Number(value);
      if (questionId === "drink_gate_q1" && Number(value) !== 3) {
        delete state.answers["drink_gate_q2"];
      }
    },
    answerCurrentQuestion(state, option, optionIndex, question) {
      const currentQuestion = question || this.getCurrentQuestion(state);
      if (!currentQuestion) return;
      this.updateAnswer(state, currentQuestion.id, option.value);
    ...
    isComplete(state) {
      const visibleQuestions = this.getVisibleQuestions(state);
      return visibleQuestions.length > 0 && visibleQuestions.every((question) => state.answers[question.id] !== undefined);
    },
    getProgressText(state) {
      return `第 ${Math.min(state.currentIndex + 1, this.getQuestionCount(state))} / ${this.getQuestionCount(state)} 题`;
    },
    getProgressHint(state) {
      const currentQuestion = this.getCurrentQuestion(state);
      return currentQuestion ? this.getQuestionMeta(currentQuestion) : "结果生成中";
    },
    computeResult(state) {
      const rawScores = {};
      const levels = {};
      ...
      const drunkTriggered = state.answers[DRUNK_TRIGGER_QUESTION_ID] === 2;
      ...
      return {
        cache: {
          title: `${finalType.code}（${finalType.cn}）`,
          desc: finalType.desc,
          badge,
          imageSrc: TYPE_IMAGES[finalType.code] || "",
          savedAt: Date.now(),
          extra: { typeCode: finalType.code }
        },
        title: `${finalType.code}（${finalType.cn}）`,
        kicker: modeKicker,
        subtitle: sub,
        ...
        badge,
        imageSrc: TYPE_IMAGES[finalType.code] || "",
        imageAlt: `${finalType.code}（${finalType.cn}）`,
        note,
        sections: [
          {
            title: "该人格的简单解读",
          ...
          },
          {
            title: "十五维度评分",
            type: "list",
            items: dimensionOrder.map((dim) => {
              const level = levels[dim];
              return {
                label: dimensionMeta[dim].name,
                value: `${level} / ${rawScores[dim]}分`,
                description: DIM_EXPLANATIONS[dim][level]
              };
            })
          },
          {
